...
export default function MessageViewQ({ route, navigation }){
    ...
    React.useEffect(() => {
        navigation.addListener('beforeRemove', (e) => {
            ...
            if(this.conversation.indexOf(this.user) == -1){
        ...
        const unsubscribe = navigation.addListener('focus', () => {
            ...
            DeviceEventEmitter.addListener("message", async (message, info)=> {
                ...
                if(niv != info[0]){
                    ...
                    clvl(info[0])
                    niv = info[0]
                    if(!this.conversation.includes(this.user) && niv > 0 && !this.alert){
                        this.alert = true
                        Alert.alert(
                            "Sauvegarde",
                            "Voulez vous sauvegarder la conversation ?",
                            [
                                { 
                                    text: "oui",
                                    onPress: () => {
                                        this.socket.emit('addConv', this.ticket, this.user, this.id)
                                        this.alert = false
                                    }
                                },
                                { 
            ...
            changeQuestion(route.params.question)
            if(this.conversation.indexOf(this.user) != -1){
                this.conversation = this.conversation.filter( x=> x != this.user);
                delete this.story[this.user]
                let temp = JSON.stringify(this.story)
                SecureStore.setItemAsync("story", temp);
            }
        });
        return unsubscribe;
    }, [navigation])
